const startYear = -5000; // Prehistory
    const endYear = 2025;
    ...
    const historicalPeriods = [
        ...
        { name: 'XXIe siècle', startYear: 2000, endYear: 2025, image: './img/XXIcentury.png' }
    ];

    const timelineWidth = container.clientWidth; // Largeur réelle du conteneur
    ...
    function updateDate() {
        let ratio = cursorX / timelineWidth;
    ...
    setInterval(() => {
      ...
      cursorX = Math.max(0, Math.min(timelineWidth, cursorX));
      cursor.style.left = cursorX + 'px';
      updateDate();
    }, 200);
